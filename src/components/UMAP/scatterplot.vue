...
//------- watch --------
// repaint when the color map object itself changes
...
function createShaderProgram(gl: WebGLRenderingContext, vsSource: string, fsSource: string) {
  ...
  if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) {
    console.error('Vertex shader error:', gl.getShaderInfoLog(vs)); return null
  }
  const fs = gl.createShader(gl.FRAGMENT_SHADER)!
  gl.shaderSource(fs, fsSource)
  gl.compileShader(fs)
  if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) {
    console.error('Fragment shader error:', gl.getShaderInfoLog(fs)); return null
  }
  ...
  if (!gl.getProgramParameter(prog, gl.VALIDATE_STATUS)) {
    console.error('Program validate error:', gl.getProgramInfoLog(prog)); return null
  }
  return prog
}
